...
fun cableEvents(fixture: String, channel: ChannelIdentifier = anAlgorithmIdentifier()): List<CableEvent> =
...
fun aCableEvent(rawText: String, channel: ChannelIdentifier = anAlgorithmIdentifier()): CableEvent =
...
fun aBroadcastFrame(message: String, channel: ChannelIdentifier = anAlgorithmIdentifier()): String = buildJsonObject {
    put("identifier", channel.asJson())
    put("message", Json.parseToJsonElement(message))
}.toString()

private fun emittedBy(rawText: String, channel: ChannelIdentifier): CableEvent? =
    (SubscriptionProtocol(channel).next(rawText) as? SubscriptionProtocol.Step.Emit)?.event
