<!-- translated-from: bootstrap.md@2fc5f1dd6b892196f8c8afca8f633632ba1f1dcc -->
...
**버전을 올렸는데 릴리스 노트에 있던 게 안 보인다면?** 기록 저장소의 `.gitignore` 를 보세요.
서버는 **없는 규칙만 추가하고, 있는 규칙은 고치지도 지우지도 않습니다** — 그 파일은 사용자의
것이고, 사용자가 내린 결정을 도구가 덮어쓰는 쪽이 규칙 하나 낡은 것보다 나쁩니다. 대신 옛
버전이 쓴 `.gitignore` 는 그 버전의 규칙을 그대로 유지한다는 비용이 따라옵니다.

한 줄은 다시 볼 만합니다. 이 규칙이 좁혀지기 전에 설치했다면 `.obsidian/` **전체**를
무시하고 있고, 그러면 그래프 색상 그룹을 비롯한 볼트 설정이 전부 같이 빠집니다. 지금 규칙은
`.obsidian/workspace.json` 과 모바일 쌍둥이만 지목하므로, 다른 기기에 클론한 볼트가 빈 상태가
아니라 설정된 상태로 열립니다. 그걸 원하면 `.obsidian/` 줄을 직접 지우세요 — 다음 시작 때
좁혀진 규칙이 추가됩니다. 상위 경로 규칙이 이미 덮고 있으면 서버는 거기서 멈추기 때문입니다.

**Obsidian을 안 쓰더라도 하나는 확인할 만합니다.** 기록을 IntelliJ로 열어본 적이 있다면 루트에
`<이름>.iml` 이 있는지 보세요:
...
ignore 규칙은 git이 이미 추적 중인 파일에는 아무 효력이 없습니다. 그래서 나중에 추가된 규칙은
이미 들어간 파일을 구제하지 못하고, 서버가 대신 추적 해제를 해주지도 않습니다 — 사용자의
저장소에 `git rm --cached` 를 실행하는 도구는 더 이상 ignore 규칙이 아니니까요. 이건 손이 갈
값을 합니다. 그 파일은 IntelliJ가 인덱싱한 모든 문제 디렉토리를 나열하기 때문에 **자기가
가리키는 기록보다 오래 살아남습니다.** 이 문제가 발견된 저장소에서는 이력을 전부 지운 뒤에도
문제 제목 4개가 그대로 올라가 있었습니다. 지우려면:

```bash
git rm --cached <이름>.iml && git commit -m "chore: untrack the IDE module file"
```
